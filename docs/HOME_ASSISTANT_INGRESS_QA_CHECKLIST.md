...
- In `View`, change Width/Depth and verify those bounds remain local to that browser. Then save a shared 3D Height min/max and verify the floor reference and visible-height FOV update without changing any configured zone coordinates. Reversed values should normalize and blank/equal/out-of-range values should be rejected without replacing the prior shared value.
- Open a second browser session and verify the saved Height appears and updates live there. Switch A → B → A, discover another compatible switch, reload the page, and restart/update the add-on; every view must retain the same shared Height while X/Y remain browser-local. With the selected switch offline, confirm the height save still works and sends no MQTT/device command.
- With no server-side shared range yet, select a switch that has a valid legacy browser-only height and verify it is offered as an unsaved draft without any automatic server write. Only `Save Shared Height` may make it global; concurrent tabs must preserve their drafts and reconcile against the latest revision. After a confirmed save, device changes must not reload old per-device values.
...
- In the Zone Editor, verify the `?` helper is collapsed by default and exposes one shared explanation for Detection Areas, Stay Areas, or Interference Areas. Changing numbered slots within a family must keep the same guidance; changing families while open must update the color, copy, caveat, and community link without moving focus or sending a socket command.
- At desktop, 700 px, and 390 px widths, verify the helper opens and closes by mouse, touch, Enter, and Space; `aria-expanded` tracks visibility; guidance wraps without horizontal overflow; links are keyboard operable; zone controls remain at least 44 px on touch/coarse-pointer layouts; and the six coordinate inputs have explicit labels and the immediate-write/editing description.
- Press `Force Sync` with a zone draft open and verify discard confirmation appears.

## 5. Configuration and Confirmation

- Open a dashboard card before fresh device reports arrive. Verify Load & Dimming shows its controls, cached presence and saved zones are restored, and unreported area occupancy reads `UNKNOWN`. Repeat after changing devices and reconnecting.
- Change a runtime option such as `power_calibration` alongside a device parameter. Confirm the option uses `bridge/request/device/options`, the parameter uses `/set`, and both confirmations are required. A bridge error or missing response must leave unresolved changes retryable. Existing options must load regardless of the order of retained `bridge/info` and `bridge/devices` messages.
- Send a maintenance command on switch A and immediately select B. Confirm only A receives the command, and a late result for A cannot clear B's pending command or change B's status.
- While a power-only header command is awaiting confirmation, deliver a newer brightness report and fail the power command. Brightness must keep the new reported value; power must restore the latest authoritative value.
- After authoritative inventory excludes a disabled, unsupported, or interview-incomplete device, replay its retained state report and reconnect MQTT. The excluded device must remain absent until a new eligible inventory entry arrives.
