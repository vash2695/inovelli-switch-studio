...
- Confirm detection, stay, and interference zones render as complete, correctly bounded translucent cuboids, including a packet where an empty raw area slot precedes an active area. Extend zones past the configured X, Y, and Z display limits and verify the 3D axes expand with padding to show each cuboid in full while the configured floor grid and saved display limits remain unchanged; the complete Z axis should retain the scene's full physical height.
