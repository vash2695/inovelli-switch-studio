...
- In `View`, set a device-specific 3D Height min/max, apply the display range, and verify the configured room grid updates without changing the physical FOV or any zone configuration. Reversed values should normalize, blank/equal/out-of-range values should be rejected, and switching A → B → A should restore each switch's own range.
...
- With the grid enabled, confirm the 3D scene has no ceiling plane and that the remaining wall grids stay visually subordinate to targets, zones, and FOV shading.
